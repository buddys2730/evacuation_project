    ...
    >
      ...
      >
        ...
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            onUpdateCrowd(editingCrowd);
          }}
        >
          <b>混雑度：</b>
          <select
            value={editingCrowd}
            onChange={(e) => setEditingCrowd(e.target.value)}
          >
        ...
        <div style={{ margin: "14px 0" }}>
            ...
            {(supplies || []).map((s) => (
              <li key={s.id}>
                ...
                <input
                  ...
                  value={s.quantity}
                  onChange={(e) =>
                    onUpdateSupply(s.id, s.item_name, e.target.value)
                  }
                  style={{ width: 60 }}
                />{" "}
                個
          ...
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              onAddSupply(newItemName, newQuantity);
              setNewItemName("");
              setNewQuantity("");
            }}
          >
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all(
      pageItems.map((s) =>
        fetch(
          `${API_BASE}/api/admin/supplies?shelter_id=${s.id}&date=${selectedDate}`,
        )
          .then((r) => r.json())
          .then((data) => ({
            id: s.id,
    ...
    Promise.all(
      pageItems.map((s) =>
        fetch(
          `${API_BASE}/api/admin/crowd?shelter_id=${s.id}&date=${selectedDate}`,
        )
          .then((r) => r.json())
  ...
  const handleDeleteSupply = (supplyId) => {
    fetch(`${API_BASE}/api/admin/supplies`, {
      ...
      setSuppliesMap((m) => ({
        ...m,
        [detailShelter.id]: (m[detailShelter.id] || []).filter(
          (s) => s.id !== supplyId,
        ),
      }));
      setTimeout(() => handleSearch(), 300);
  ...
  return (
    <div style={{ padding: 32, fontFamily: "sans-serif" }}>
      ...
      <div>
        ...
        <button style={{ marginLeft: 12 }} onClick={handleSearch}>
          検索
        </button>
      </div>
          ...
          <thead style={{ background: "#e3ecfc" }}>
            <tr>
              ...
              <th
                ...
                onClick={() => {
                  setSortKey("address");
                  setSortAsc((k) => (sortKey === "address" ? !k : true));
                }}
              >
              ...
              <th
                style={{ cursor: "pointer" }}
                onClick={() => {
                  setSortKey("elevation");
                  setSortAsc((k) => (sortKey === "elevation" ? !k : true));
                }}
              >
              ...
              <tr key={s.id}>
                ...
                <td>
                  {suppliesMap[s.id] && suppliesMap[s.id].length > 0
                    ? suppliesMap[s.id]
                        .map(
                          (item) => `${item.item_name}（${item.quantity}個）`,
                        )
                        .join("，")
                    : "未登録"}
                </td>
