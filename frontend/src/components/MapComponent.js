...
const MapComponent = ({
  ...
  useEffect(() => {
    ...
    Promise.all(
      normalizedPoints.map(s =>
        fetch(`${process.env.REACT_APP_API_BASE_URL}/api/supplies?shelter_id=${s.id}`)
          .then(r => r.json())
      ...
      normalizedPoints.forEach(p => { p.supplies = map[p.id] || []; });
    });
    Promise.all(
      normalizedPoints.map(s =>
        fetch(`${process.env.REACT_APP_API_BASE_URL}/api/crowd?shelter_id=${s.id}`)
          .then(r => r.json())
      ...
      normalizedPoints.forEach(p => { p.crowdedness = map[p.id] || "未登録"; });
    });
  ...
  const point = normalizedPoints.find(p => String(p.id) === String(selectedId));
  if (!point) {
