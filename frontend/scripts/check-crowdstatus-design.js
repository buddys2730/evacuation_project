import fs from "fs";
const MD_PATH = "./docs/CrowdStatus.md";
const JS_PATH = "./src/components/CrowdStatus.js";
...
const propsArgsMatch = js.match(/CrowdStatus\s*=\s*\(\s*{([^}]*)}/s);
const jsProps =
  propsArgsMatch
    ? propsArgsMatch[1].split(",").map((s) => s.replace(/=.*$/, "").trim()).filter(Boolean)
    : [];
const propTypesMatch = js.match(/CrowdStatus\.propTypes\s*=\s*{([^}]*)}/s);
...
if (missingInJs.length === 0 && missingInMd.length === 0) {
  console.log("✅ CrowdStatus.mdとコードのpropsが完全一致しました。");
  process.exit(0);
} else {
