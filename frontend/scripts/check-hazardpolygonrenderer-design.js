import fs from "fs";
const MD_PATH = "./docs/HazardPolygonRenderer.md";
const JS_PATH = "./src/components/map/HazardPolygonRenderer.js";
...
const propsArgsMatch = js.match(/HazardPolygonRenderer\s*=\s*\(\s*{([^}]*)}/s);
const jsProps =
  propsArgsMatch
    ? propsArgsMatch[1].split(",").map((s) => s.replace(/=.*$/, "").trim()).filter(Boolean)
    : [];
const propTypesMatch = js.match(/HazardPolygonRenderer\.propTypes\s*=\s*{([^}]*)}/s);
...
if (missingInJs.length === 0 && missingInMd.length === 0) {
  console.log("✅ HazardPolygonRenderer.mdとコードのpropsが完全一致しました。");
  process.exit(0);
} else {
