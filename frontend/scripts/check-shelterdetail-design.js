import fs from "fs";
const MD_PATH = "./docs/ShelterDetail.md";
const JS_PATH = "./src/components/ShelterDetail.js";
...
const propsArgsMatch = js.match(/ShelterDetail\s*=\s*\(\s*{([^}]*)}/s);
const jsProps =
  propsArgsMatch
    ? propsArgsMatch[1].split(",").map((s) => s.replace(/=.*$/, "").trim()).filter(Boolean)
    : [];
const propTypesMatch = js.match(/ShelterDetail\.propTypes\s*=\s*{([^}]*)}/s);
...
if (missingInJs.length === 0 && missingInMd.length === 0) {
  console.log("✅ ShelterDetail.mdとコードのpropsが完全一致しました。");
  process.exit(0);
} else {
