...
const mdProps = md.split("\n").filter(line => /^\|\s*`[^`]+`\s*\|/.test(line)).map(line => {
  const m = line.match(/^\|\s*`([^`]+)`/); return m ? m[1] : null;
}).filter(Boolean);
...
const propProps = propTypesMatch
  ? propTypesMatch[1].split(",").map((s) => s.split(":")[0].trim()).filter(Boolean)
  : [];
...
if (missingInJs.length === 0 && missingInMd.length === 0) {
  ...
  if (missingInJs.length > 0) console.error("❌ mdにあるがJSにないprops:", missingInJs.join(", "));
  if (missingInMd.length > 0) console.error("❌ JSにあるがmdにないprops:", missingInMd.join(", "));
  process.exit(1);
}
