import fs from "fs";
const MD_PATH = "./docs/ShelterSupplies.md";
const JS_PATH = "./src/components/ShelterSupplies.js";
...
const propsArgsMatch = js.match(/ShelterSupplies\s*=\s*\(\s*{([^}]*)}/s);
const jsProps =
  propsArgsMatch
    ? propsArgsMatch[1].split(",").map((s) => s.replace(/=.*$/, "").trim()).filter(Boolean)
    : [];
const propTypesMatch = js.match(/ShelterSupplies\.propTypes\s*=\s*{([^}]*)}/s);
...
if (missingInJs.length === 0 && missingInMd.length === 0) {
  console.log("✅ ShelterSupplies.mdとコードのpropsが完全一致しました。");
  process.exit(0);
} else {
