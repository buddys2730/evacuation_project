import fs from "fs";
const MD_PATH = "./docs/RouteRenderer.md";
const JS_PATH = "./src/components/map/RouteRenderer.js";
...
const propsArgsMatch = js.match(/RouteRenderer\s*=\s*\(\s*{([^}]*)}/s);
const jsProps = propsArgsMatch
  ? propsArgsMatch[1]
      ...
      .filter(Boolean)
  : [];
const propTypesMatch = js.match(/RouteRenderer\.propTypes\s*=\s*{([^}]*)}/s);
...
if (missingInJs.length === 0 && missingInMd.length === 0) {
  console.log("✅ RouteRenderer.mdとコードのpropsが完全一致しました。");
  process.exit(0);
} else {
