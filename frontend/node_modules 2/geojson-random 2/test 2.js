...
test('random.polygon', function (t) {
  ...
  t.equal(
    randomPolygons.features[0].geometry.coordinates[0].length,
    11,
    'and have 11 positions in their outer rings'
  );

  var randomPolygonsHi = geojsonRandom.polygon(100, 20);
  t.equal(
    randomPolygonsHi.features[0].geometry.coordinates[0].length,
    21,
    'and have 21 positions in their outer rings'
  );
  t.end();
});

test('random.polygon with bbox', function (t) {
  var randomPolygonRing = geojsonRandom.polygon(1, 5, 5, [50, 50, 60, 60]).features[0].geometry
    .coordinates[0];
